...
export const DropDown = ({title, children, selected, color = "bg-[#2a2a30]", canRotate = false}) => {
  ...
  return (
    <div className="relative">
      ...
      <div className={`absolute z-50 right-2 top-12 ${color} p-4 rounded-lg flex select-none ${open ? "opacity-100 z-50 translate-y-0" : "opacity-0 -z-50 -translate-y-10 pointer-events-none"} transition-all flex-col z-[99]`}>
        { children }
      </div>
    </div>
  )
